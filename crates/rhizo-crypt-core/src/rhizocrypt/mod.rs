...
impl RhizoCrypt {
    ...
    pub async fn branch_session(
        ...
        // Build parent→children index and id lookup for BFS backward walk
        let mut id_map: std::collections::HashMap<VertexId, &Vertex> =
        ...
        // Copy ancestors into the new session in topological order
        let mut copied: u64 = 0;
        // Topological sort: keep only visited vertices, insert in parent-first order
    ...
    pub async fn diff_sessions(
        ...
        let base_ids: std::collections::HashSet<VertexId> =
            base_vertices.iter().filter_map(|v| v.compute_id().ok()).collect();
        let other_ids: std::collections::HashSet<VertexId> =
