...
async fn test_provenance_dehydrate_alias() {
    ...
    let root = handle_request(&server, req, &test_gate(), &test_caller()).await;
...
async fn test_provenance_full_pipeline_via_aliases() {
    ...
    let info = handle_request(&server, req, &test_gate(), &test_caller()).await.unwrap();
    assert_eq!(info["vertex_count"], 1);

    let req = make_request("provenance.session.list", None);
    let list = handle_request(&server, req, &test_gate(), &test_caller()).await.unwrap();
