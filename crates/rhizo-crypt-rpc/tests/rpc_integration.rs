...
//!
//! All tests that start an `RpcServer` (tarpc) must clear `FAMILY_ID` /
//! `RHIZOCRYPT_FAMILY_ID` from the environment so the server runs in
//! development mode (no BTSP handshake). Gate-deployed environments set
//! `FAMILY_ID`, which would otherwise reject plain tarpc connections.
...
/// Environment variables to clear for development-mode tarpc tests.
...
#[test]
fn test_rpc_server_client_connection() {
...
#[test]
fn test_rpc_genesis_and_children() {
    let rt = build_runtime();
    temp_env::with_vars(BTSP_CLEAR_ENV, || {
        rt.block_on(async {
            let (_primal, client, server_handle) = setup_server_client().await;
            ...
            let children =
                client.get_children(session_id, genesis_id).await.expect("should get children");
...
fn test_rpc_merkle_operations() {
    ...
    temp_env::with_vars(BTSP_CLEAR_ENV, || {
        rt.block_on(async {
            ...
            let proof = client
                .get_merkle_proof(session_id, vertex_id)
                .await
                .expect("should get merkle proof");
...
fn test_rpc_slice_operations() {
    ...
    temp_env::with_vars(BTSP_CLEAR_ENV, || {
        rt.block_on(async {
            ...
            let slice_id =
                client.checkout_slice(checkout_request).await.expect("should checkout slice");
...
fn test_rpc_dehydration() {
    ...
    temp_env::with_vars(BTSP_CLEAR_ENV, || {
        rt.block_on(async {
            ...
            let status =
                client.get_dehydration_status(session_id).await.expect("should get status");
